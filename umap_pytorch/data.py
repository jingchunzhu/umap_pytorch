...
class UMAPDataset(Dataset):
    def __init__(self, data, graph_, n_epochs=200):
        ...
        self.total = epochs_per_sample.sum()
        '''
    ...
    def __getitem__(self, index):
        # Instead of expanding, sample one edge
        edge_idx = np.random.choice(len(self.head), p=self.epochs_per_sample / self.total)
        return self.data[self.head[edge_idx]], self.data[self.tail[edge_idx]]
